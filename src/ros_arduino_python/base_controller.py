...
>>>>>>> b56f3d7e657742da3efd0bc5a198a7c0bbc2a3b8
        ...
        pid_params['Kp'] = rospy.get_param("~Kp", 30)
        pid_params['Kd'] = rospy.get_param("~Kd", 24)
        pid_params['Ki'] = rospy.get_param("~Ki", 0)
        pid_params['Ko'] = rospy.get_param("~Ko", 75)
...
<<<<<<< HEAD
        self.encodergoalPub= rospy.Publisher('Encodergoal',Twist,queue_size=5)
    ...
    def poll(self):
        ...
        if now > self.t_next:
            ...
            if not self.stopped:
                ...
                encoder_goal=Twist()
                encoder_goal.linear.x = self.v_left*self.arduino.PID_RATE
                encoder_goal.linear.y = self.v_right*self.arduino.PID_RATE
                self.encodergoalPub.publish(encoder_goal)
